...
License: GPL 2.0
Translated into spanish by Rafael Poveda - RaveN (bi0xid)
	...
	function form( $instance ) {
		print(
			...
			'<p style="line-height: 105%; text-align: center;">
				<small>
					Pasa el ratón por encima de los textos para ver la ayuda contextual
				</small>
			</p>'."\r\n");
		...
		$instance = wp_parse_args( $instance, $defaults ); 
		 ...
		 <!-- BASIC OPTIONS -->
		 <div id="afss_basic_button" onclick="jQuery('.afss_options').slideUp('fast',function(){ jQuery('div#afss_basic').slideDown(function(){jQuery(this).stop();});});" class="button" style="text-align: center; margin: 5px auto 0; clear: right;">Opciones básicas</div>
		 <div id="afss_basic" class="afss_options">
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>" title="Este texto aparecerá como título en la barra lateral.">Título:</label>
			<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" style="width:90%;" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'feed_url' ); ?>" title="La URL del feed que tenemos que leer.">URL del feed:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'target' ); ?>" title="Dónde se abren los enlaces al pinchar sobre las imágenes.">Los enlaces abre en:</label>
			<select id="<?php echo $this->get_field_id( 'target' ); ?>" name="<?php echo $this->get_field_name( 'target' ); ?>" class="widefat" style="width:90%;">
				<option value="_blank" <?php if ( '_blank' == $instance['target'] ) echo 'selected="selected"'; ?>>Nueva ventana</option>
				<option value="_self" <?php if ( '_self' == $instance['target'] ) echo 'selected="selected"'; ?>>Misma ventana</option>
			</select>
		</p>
		...
		<!-- DISPLAY OPTIONS -->
		<div id="afss_display_button" onclick="jQuery('.afss_options').slideUp('fast',function(){ jQuery('div#afss_display').slideDown(function(){jQuery(this).stop();});});" class="button" style="text-align: center; margin: 5px auto 0;">Opciones de visualización</div>
		<div id="afss_display" class="afss_options" style="display: none;">
		<table width="95%"><tr><td width="50%">
			<p>
				<label for="<?php echo $this->get_field_name( 'height' ); ?>" title="La altura del widget">Altura:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'width' ); ?>" title="El ancho del widget">Ancho:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'loading_text' ); ?>" title="El texto que aparece mientras que las imágenes se cachean">Texto de carga:</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_navigation' ); ?>" title="Mostrar o no las flechas para navegar entre imágenes cuando pasamos el ratón por encima">¿Mostrar navegación?</label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_titlebar' ); ?>"title="Mostrar o no el título al pasar el ratón por encima">¿Mostrar título?</label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'perm_titlebar' ); ?>"title="Eliminar el título cuando apartas el ratón">¿Título permanente?</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'bgcolor' ); ?>" title="El color de fondo del slideshow">Color de fondo:</label>
		...
		<!-- FEED OPTIONS -->
		<div id="afss_feed_button" onclick="jQuery('.afss_options').slideUp('fast',function(){ jQuery('div#afss_feed').slideDown(function(){jQuery(this).stop();});});" class="button" style="text-align: center; margin: 5px auto 0;">Opciones del feed</div>
		<div id="afss_feed" class="afss_options" style="display: none;">
		
		<p>
			<label for="<?php echo $this->get_field_id( 'media_type' ); ?>" title="Si el feed tiene contenido multimedia, selecciona si mostrar miniaturas (recomendado) o el tamaño completo. Para Flickr y Picasa es reocmendable utilizar las miniaturas.">Mostrar tamaño:</label>
			<select id="<?php echo $this->get_field_id( 'media_type' ); ?>" name="<?php echo $this->get_field_name( 'media_type' ); ?>" class="widefat" style="width:90%;">
				<option <?php if ( 'content' == $instance['media_type'] ) echo 'selected="selected"'; ?>>contenido completo</option>
				<option <?php if ( 'thumbnail' == $instance['media_type'] ) echo 'selected="selected"'; ?>>miniaturas</option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_name( 'maximages' ); ?>" title="El número máximo de imágenes a tomar del feed">Máximo de imágenes:</label>
		...
		<!-- TIMING OPTIONS -->
		<div id="afss_timing_button" onclick="jQuery('.afss_options').slideUp('fast',function(){ jQuery('div#afss_timing').slideDown(function(){jQuery(this).stop();});});" class="button" style="text-align: center; margin: 5px auto 0;">Opciones de timing</div>
		<div id="afss_timing" class="afss_options" style="display: none;">
		<table width="95%"><tr><td width="50%">
			<p>
				<label for="<?php echo $this->get_field_id( 'pause_rate' ); ?>" title="El tiempo que se pausará el slide entre foto y foto">Pausa (milisegundos):</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'fade_rate' ); ?>" title="El tiempo que durará el efecto del fading">Fading (milisegundos):</label>
